...
-- 84
lazyProduct :: [Int] -> Int
lazyProduct = product . takeWhileIncl (/= (0 :: Int))
  where
    takeWhileIncl p l =
      case l of
        [] -> []
...
-- >>> removeAt 10 [1 .. 5]
-- (Nothing,[1,2,3,4,5])
removeAt :: Integral a => a -> [b] -> (Maybe b, [b])
removeAt _ [] = (Nothing, [])
...
experienceForDragon :: DragonType -> Experience
experienceForDragon RedDragon = Experience 100
experienceForDragon BlackDragon = Experience 150
experienceForDragon GreenDragon = Experience 250

-- @chshersh If I would create different constructors for different dragon types, is there a way to share common properties between constructors?
-- Except simple composition 
...
-- TODO How to make unrepresantable the state when reward for Green dragon includes a treasure?
-- Make different constructors for different types of dragons?
rewardForDragon :: Dragon a -> Reward a
rewardForDragon Dragon { dragonType = GreenDragon, eatenChest } = Reward { 
        rewardTreasure = Nothing,
        rewardGold = chestGold eatenChest,
        rewardExp = experienceForDragon GreenDragon
    } 
rewardForDragon Dragon { dragonType, eatenChest } = Reward { 
        rewardTreasure = Just (chestTreasure eatenChest),
        rewardGold = chestGold eatenChest,
        rewardExp = experienceForDragon dragonType
    } 
...
dragonFight = go 1
    where
        ...
        go stepNum knight dragon
                | dragonHealth attackedDragon <= 0 = Victory (rewardForDragon attackedDragon)
                | knightHealth attackedKnight <= 0 = Defeat
                | knightEndurance attackedKnight <= 0 = Escape
                | otherwise = go (stepNum+1) attackedKnight attackedDragon
            where
                (knight', dragon') = doAttackDragon knight dragon
                (attackedKnight, attackedDragon) = doAttackKnight stepNum knight' dragon'
        
        doAttackDragon knight dragon = (
                knight { knightEndurance = knightEndurance knight - 1 },
                dragon { dragonHealth = dragonHealth dragon - knightAttack knight }
            )
        doAttackKnight stepNum knight dragon = 
            if stepNum `mod` 10 == 0
            then (knight { knightHealth = knightHealth knight - 1 }, dragon)
            else (knight, dragon)
...
eval vars (Var varName) =
  case lookup varName vars of
    Just x -> Right x
    Nothing -> Left (VariableNotFound varName)
eval _ (Lit value) = Right value
eval vars (Add a b) =
  let evaluatedA = eval vars a
      evaluatedB = eval vars b
   in case (evaluatedA, evaluatedB) of
        (Left err, _) -> Left err
        (_, Left err) -> Left err
...
constantFolding = constructResult . extractLit
  where
    constructResult :: (Maybe Expr, Int) -> Expr
    constructResult (Nothing, x) = Lit x
    constructResult (Just a, 0) = a
    constructResult (Just a, x) = Add a (Lit x)

    extractLit :: Expr -> (Maybe Expr, Int)
    extractLit (Lit a) = (Nothing, a)
    extractLit (Var x) = (Just (Var x), 0)
    extractLit (Add exprA exprB) =
      let (modifiedExprA, litA) = extractLit exprA
          (modifiedExprB, litB) = extractLit exprB
          resultExpr = case (modifiedExprA, modifiedExprB) of
            (Nothing, Nothing) -> Nothing
            (Just a, Nothing) -> Just a
            (Nothing, Just b) -> Just b
            (Just a, Just b) -> Just (Add a b)
       in (resultExpr, litA + litB)
